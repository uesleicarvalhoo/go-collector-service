...
func newSut() *Publisher {
	...
	fileChannel := make(chan models.File, 10)
	waitGroup := &sync.WaitGroup{}

	return New(1, "files", storage.NewMemoryStorage(), fileChannel, eventChannel, waitGroup)
}
...
func TestStartConsumeFiles(t *testing.T) {
	// Prepare
	folder, err := ioutil.TempDir("", "*")
	if err != nil {
		panic(err)
...
func TestStartShouldSendSuccessEventWhenProcessFileReturnNil(t *testing.T) {
	// Prepare
	folder, err := ioutil.TempDir("", "*")
	if err != nil {
		panic(err)
...
func TestStartShouldSendErrorEventWhenProcessFileReturnError(t *testing.T) {
	// Prepare
	folder, err := ioutil.TempDir("", "*")
	if err != nil {
		panic(err)
